...
public $paginate = array('limit'=>10, 'order'=>array('producer_name'=>'asc'));
public $fk = null;

public function __construct( $request = NULL, $response = NULL ) {
    parent::__construct($request,$response);
    if ( empty($this->translatedSingularName) )
            $this->translatedSingularName = __('Producer',true);

}
function setRelated() {	
    $countries = $this->Producer->Country->find('list');    
...
public function ajaxGetCountryProducers() {
    $this->layout = null;
    $country_id = $this->request->data('country_id');
    if ($country_id > 0) {
        $producers = $this->Producer->find('list',array('conditions'=>array('Producer.country_id = '=>$country_id)));
